...
def main():
    ...
    ax0.annotate(
        ...
        xytext=(0, 0.25),
        arrowprops=dict(
            arrowstyle="-|>", mutation_scale=25, color="blue", lw=2.5
        ),
    )
    ax0.text(
        1.12,
    ...
    ax1.set_title(
        "Unforced reference simulation", fontsize=17, fontweight="bold"
    )
        ...
        for i_temp in range(len(runs)):
            if (
                ...
                he_sample = get_he_events(
                    runs[i_temp][i_smb], args.region, flux_thresh
                )
                ...
                theta = 2 * np.pi / 1500.0 * time
                plot_he_lines(
                    ax[count_row][count_col], theta, vol, surge_events
                )
                plot_do_cycle_polar(ax[count_row][count_col])
                set_axis_properties(
                    args.region, ax[count_row][count_col], xticks, labels
                )
                create_subplot_labels(
                    ax[count_row][count_col], count_row, count_col
                )
                add_phase_locking_text(ax[count_row][count_col], sig_level)
                if count_col == 0:
                    plot_panel_titles(
                        ax[count_row][count_col], smb_pert[i_smb], 0
                    )
                if count_row == 1:
                    plot_panel_titles(
                        ax[count_row][count_col], t_pert[i_temp], 1
                    )
                if count_col == 1:
                    count_col = 0
                    count_row += 1
                    ...
                    count_col += 1
    plt.savefig(
        f"Fig02_Polar_{args.region}_Reduced.png", dpi=300, bbox_inches="tight"
    )
