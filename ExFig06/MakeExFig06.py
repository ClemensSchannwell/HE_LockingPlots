...
def main():
    ...
    temp, _, _, = read_var(fname1, "ice_surface_temp")
    temp = temp.transpose()
    smb, _, _, = read_var(fname1, "climatic_mass_balance")
    ...
    for i_count, i_ax in enumerate(ax):
        ...
        i_ax.gridlines(draw_labels=True, linewidth=0.5, linestyle='--',
                       color='black')
        pc = i_ax.pcolormesh(lon, lat, pl_vars[i_count],
                             transform=ccrs.PlateCarree(), cmap=cmapD,
                             vmin=-var_max[i_count], vmax=var_max[i_count])
        i_ax.contour(lon, lat, thk, clevs, transform=ccrs.PlateCarree(),
                     colors=['black'])
