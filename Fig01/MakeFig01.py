...
def main():
    ...
    for count, val in enumerate(snaps):
        ...
        ax[count].fill_between(
            distance - 100,
            -500 * np.ones(len(distance)),
            np.zeros(len(distance)),
            color="lightblue",
        )
        polygon = ax[count].fill_between(
            distance - 100, zb_smooth, usurf_smooth, color="none"
        )
        verts = np.vstack([p.vertices for p in polygon.get_paths()])
        gradient = ax[count].imshow(
        ...
        gradient.set_clip_path(
            polygon.get_paths()[0], transform=ax[count].transData
        )
        ax[count].fill_between(
            distance[start - 1 : end] - 100,
            zb_smooth[start - 1 : end] - 10,
            zb_smooth[start - 1 : end] + 100,
            color="red",
        )
        ax[count].fill_between(
            distance[0:start] - 100,
            zb_smooth[0:start] - 10,
            zb_smooth[0:start] + 100,
            color="blue",
        )
        ax[count].fill_between(
            distance[ocean_sindex:] - 100,
            zb_smooth[ocean_sindex:],
            bed_smooth[ocean_sindex:],
            color="lightblue",
        )
        ax[count].fill_between(
            distance - 100,
        ...
        axins2 = inset_axes(
            ...
            width="65%",
            height="52%",
            bbox_to_anchor=(0.41, 0.2, 0.55, 0.8),
            bbox_transform=ax[count].transAxes,
            loc=1,
        )
