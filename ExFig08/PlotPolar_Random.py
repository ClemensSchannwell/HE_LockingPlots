...
def create_subplot_labels(ax,row,col):
    PLabelMat = [['(a)', '(b)', '(c)', '(d)'],['(e)', '(f)', '(g)', '(h)'],
                 ...
                 ['(q)', '(r)', '(s)', '(t)'], ['(u)', '(v)', '(w)', '(x)'],
                 ['(y)', '(z)', '(aa)', '(bb)']]
    ax.text(0.00,1.00, PLabelMat[row][col],color='black',
...
def get_he_timing(Run, phase, surge_events):
    ...
    max_ind = [surge_events[isurge][np.argmax(F[surge_events[isurge]])] for isurge in
               range(len(surge_events))]
...
def plot_panel_titles(ax,iTemp):
    ax.set_title("$\Delta$Temp = " + str(TPert[iTemp])+"°C", \
...
def Main():
    ...
    for iMult in range(len(Runs)):
        ...
        # print(surge_events)
        # print(T)
        NCFile=Dataset("/work/ba0989/m300792/HE_Runs/ExperimentsComposite/HE112/random_forcing.nc")
    ...
    plt.savefig('Polar_Random_' + Region + '.png', dpi=300, bbox_inches='tight')
    plt.show()
