...
def main():
    ...
    for i_clen in clen:
        if args.region == "Kenzie":
            base_sample = get_he_events(
                "HE02", args.region, threshold=1.0e12, clen=i_clen
            )
            grad_thresh = -2e11
            flux_thresh = 1.5e12
            figno = "05"
        ...
        for i_smb in range(len(runs[0])):
            for i_temp in range(len(runs)):
                ...
                he_sample = get_he_events(
                    runs[i_temp][i_smb], args.region, flux_thresh, clen=i_clen
                )
                ...
                set_axis_properties(
                    args.region, ax[i_smb][i_temp], xticks, labels
                )
                plot_panel_titles(
                    ax[i_smb][i_temp], i_smb, i_temp, t_pert, smb_pert
                )
                create_subplot_labels(ax[i_smb][i_temp], i_smb, i_temp)
        ax[0, 0].legend(loc=1, bbox_to_anchor=(1.25, 1.1))
        plt.savefig(
            f"ExFig{figno}_Polar_{args.region}_{i_clen}.png",
            dpi=300,
            bbox_inches="tight",
        )
    plt.show()
