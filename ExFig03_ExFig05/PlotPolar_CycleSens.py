...
def plot_panel_titles(ax, iSMB, iTemp):
    ...
    if iTemp == 0:
        SMBStr = (
            "$\Delta$SMB = " + str(round(SMBPert[iSMB] / 1000, 3)) + " m/yr"
        )
        ax[iSMB][iTemp].text(
            -0.2,
            0.5,
            ...
            horizontalalignment="center",
            verticalalignment="center",
            transform=ax[iSMB][iTemp].transAxes,
            rotation=90,
            fontsize=14,
            fontweight="bold",
        )
...
def Main():
    ...
    parser.add_argument(
        ...
        help="Specify which region you want to plot",
    )
    parser.add_argument(
        "-clen",
        "--cyclelen",
        choices=["1000", "2000"],
        help="Specify DO cycle length",
    )
    ...
    for iSMB in range(len(Runs[0])):
        for iTemp in range(len(Runs)):
            ...
            global T
            T, Vol = ReadData(File)
            HESample = get_he_events(
                Runs[iTemp][iSMB], Threshold=FluxThresh, CLen=CLen
            )
    ...
    plt.savefig(
        "Polar_" + args.cyclelen + "_" + Region + ".png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
